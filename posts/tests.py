...
class TestScriptUser(TestCase):
    ...
    def test_profile(self):
        ...
        self.assertEqual(response.context["requested_user"].username, self.user.username)

    def test_new_post(self):
        response = self.client.post(reverse('new_post'), {'text': 'Привет ревьюерам!'}, follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_post_edit(self):
        ...
        response = self.client.post(reverse('post_edit', args=[self.user.username, self.post.id]), {'text': new_text}, follow=True)
